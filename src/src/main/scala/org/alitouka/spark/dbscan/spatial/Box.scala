...
/** Represents a box-shaped region in multi-dimensional space
  ...
  */
private[dbscan] case class Box(bounds: Array[BoundsInOneDimension], boxId: BoxId = 0, partitionId: Int = -1, var adjacentBoxes:List[Box] = Nil)
  ...
  def splitAlongLongestDimension(numberOfSplits: Int, idGenerator: BoxIdGenerator = new BoxIdGenerator(this.boxId)): Iterable[Box] = {
